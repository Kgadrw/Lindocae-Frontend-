          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {products.map((product, index) => {
                console.log(product);
                return (
                  <tr key={product._id} className={`transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-green-50'}`}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10">
                          {Array.isArray(product.image) && product.image.length > 0 ? (
                            <img src={product.image[0].url || product.image[0]} alt={product.name} className="h-10 w-10 rounded-lg object-cover border border-gray-200" />
                          ) : Array.isArray(product.images) && product.images.length > 0 ? (
                            <img src={product.images[0].url || product.images[0]} alt={product.name} className="h-10 w-10 rounded-lg object-cover border border-gray-200" />
                          ) : typeof product.image === 'string' && product.image ? (
                            <img src={product.image} alt={product.name} className="h-10 w-10 rounded-lg object-cover border border-gray-200" />
                          ) : (
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-sm text-gray-500 truncate max-w-xs">{typeof product.description === 'string' ? product.description : JSON.stringify(product.description)}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {(() => {
                        ...
                        const categoryName = Array.isArray(categories) 
                          ...
                          : null;
                        return categoryName || (typeof product.category === 'string' ? product.category : product.category?.name || 'Unknown');
                      })()}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      ${product.price}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-xs text-gray-500">{product.stockType}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-2 rounded-lg shadow hover:from-blue-600 hover:to-blue-700 transition-all"
                        >
                          <Edit size={14} />
                        </button>
                        <button
                          onClick={() => handleDelete(product._id)}
                          className="bg-gradient-to-r from-red-500 to-red-600 text-white p-2 rounded-lg shadow hover:from-red-600 hover:to-red-700 transition-all"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
